...
def csv_table_config_to_dict(c: CsvTableConfig) -> dict:
    """Serialize a CsvTableConfig to a dict for Airflow XCom."""
    return {
        "id": c.id,
...
def csv_table_config_from_dict(d: dict) -> CsvTableConfig:
    """Reconstruct a CsvTableConfig from an XCom dictionary."""
    return CsvTableConfig(
        id=d["id"],
        ...
        cursor_column=d.get("cursor_column", ""),
        initial_value=d.get("initial_value", ""),
        primary_key=d.get("primary_key", ""),
        load_sequence=d.get("load_sequence", 0),
        table_load_active=d.get("table_load_active", True),
    )


def table_config_from_dict(d: dict) -> TableConfig:
    """Reconstruct a TableConfig from an XCom dictionary."""
    return TableConfig(
        name=d["table_name"],
        load_strategy=d["load_strategy"],
        data_subject=d["data_subject"],
        cursor_column=d.get("cursor_column") or None,
        initial_value=d.get("initial_value") or None,
        primary_key=[d["primary_key"]] if d.get("primary_key") else None,
    )
